...
export const EditPaymentStatusModel = (props) => {
  ...
  const {
    ...
    mutationFn: async (status: { status: string }) => {
      return axios.patch(
        `${SERVER_URI}/api/admin/orders/${id}/payment-status`,
        status,
        {
          headers: {
          ...
          <Controller
            ...
            render={({ field }) => (
              <Autocomplete
                ...
                renderInput={(params) => (
                  <TextField {...params} label={t("Payment Status")} />
                )}
              />
            )}
          />
